...
def register(data: UserCreateAndLogin, auth: AuthService = Depends(get_auth_service)):
    auth.register(data)
...
def login(data: UserCreateAndLogin, auth: AuthService = Depends(get_auth_service)):
    token = auth.login(data)
    return {"access_token": token, "token_type": "bearer"}
