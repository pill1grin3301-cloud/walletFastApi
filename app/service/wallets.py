...
class WalletsService:
    ...
    def get_all_wallets(self, current_user) -> list[WalletORM]:
        return self.wallets_repository.get_all_by_user(current_user.id)
    ...
    def create_wallet(self, wallet: CreateWalletRequest, current_user):
        ...
        new_wallet = self.wallets_repository.create(
            wallet.name,
            wallet.initial_balance,
            current_user.id,
        )
        self.db.add(new_wallet)
        self.db.flush()  # id нужен до записи операции в историю
